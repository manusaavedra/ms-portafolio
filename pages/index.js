	...
	return (
		<div>
			...
			<main className="max-w-4xl mx-auto">
				<section className="flex h-[calc(100vh-200px)] px-4 flex-col md:flex-row items-center">
					<picture className="w-[220px] md:w-[320px]">
						<img src="/avatar.png" alt="manuel saavedra avatar" />
					</picture>
					<div className="flex flex-col max-w-md text-center md:text-left gap-2">
				...
				<div className="flex flex-col justify-center items-center gap-4">
					<h5 className="font-semibold text-xl text-center">Aquí tienes algunos proyectos que te quiero compartir.</h5>
					<button onClick={() => scrollVisibleArea("#projects")}>
						<IoIosArrowDown size={24} color="inherit" />
					</button>
				</div>
				<section className="pt-40 px-4 grid grid-rows-3 grid-cols-1 gap-10" id="projects">
					{
						projects.map((project) => (
							<article key={project.id} className="border rounded-xl py-4 grid grid-cols-1 md:grid-cols-2 place-items-center">
